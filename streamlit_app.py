...
def load_model_components():
    """Memuat model, vectorizer, dan encoder dari folder Data."""
    try:
        model = joblib.load(os.path.join("Data", "lightgbm_model.joblib"))
        vectorizer = joblib.load(os.path.join("Data", "tfidf_vectorizer.joblib"))
        encoder = joblib.load(os.path.join("Data", "label_encoder.joblib"))
...
@st.cache_data
def load_data():
    """Memuat data lengkap dari folder Data."""
    try:
        df = pd.read_csv(os.path.join("Data", "data_lengkap.csv"))
        return df
    except FileNotFoundError:
        return None
...
if filtered_df.empty:
    ...
    fig_bar = px.bar(sentiment_percentage, x=sentiment_percentage.index, y=['positif', 'negatif', 'netral'],
                     title="Distribusi Sentimen pada Setiap SDG", labels={'x': 'Kategori SDG', 'value': 'Persentase (%)'},
                     color_discrete_map={'positif': 'green', 'negatif': 'red', 'netral': 'grey'}, barmode='stack')
